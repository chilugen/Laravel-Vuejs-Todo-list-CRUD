  ...
  <div>
    ...
    <form v-on:submit.prevent = "updatePost">
      <div class="form-group">
        <label for="edit-title">Гарчиг</label>
        <input id="edit-title" v-model="post.title" class="form-control" required />
      </div>
      <div class="form-group">
        <label for="edit-body">Үндсэн мэдээ</label>
        <textarea class="form-control" rows="10" v-model="post.body"></textarea>
      </div>
      <button type="submit" class="btn btn-xs btn-primary">Нэмэх</button>
      <router-link class="btn btn-xs btn-warning" v-bind:to="'/'">Болих</router-link>
    </form>
  </div>
</template>
